/*
*    Class implementation of the SoC Designer component <SD_AXIv2_DummyGraphicsMacroModel>
*
...
SD_AXIv2_DummyGraphicsMacroModel::SD_AXIv2_DummyGraphicsMacroModel(CASIModuleIF* c, const string &s) : CASIModule(c, s)
{
	uint32_t i;
	char strBuffer[1000];

	AXIv2_master_TMaster = new DummyGraphicsMacroModel_AXI_Master_Port<SD_AXIv2_DummyGraphicsMacroModel>( this, "AXIv2_master" );
	...
	for ( i = 0; i < 4; i++ )
	{
		...
		DummyGraphicsMacroModel_interrupt_SSlave[i] = new DummyGraphicsMacroModel_interrupt_SS<SD_AXIv2_DummyGraphicsMacroModel>( this, i );
	...
	DummyGraphicsMacroModel_reset_x_SSlave = new DummyGraphicsMacroModel_reset_x_SS<SD_AXIv2_DummyGraphicsMacroModel>( this );
  ...
  defineParameter( "Enable Debug Messages",                     "false", eslapi::CASI_PARAM_BOOL);
	...
	defineParameter( "number_of_thread",                              "5", eslapi::CASI_PARAM_VALUE,  0);
...
SD_AXIv2_DummyGraphicsMacroModel::~SD_AXIv2_DummyGraphicsMacroModel()
{
	uint32_t i;
...
void SD_AXIv2_DummyGraphicsMacroModel::communicate()
{
...
void SD_AXIv2_DummyGraphicsMacroModel::update()
{
	// the following message will be printed only in the debug version of MxExplorer
	if (p_enableDbgMsg == true)
	{
...
/* FSL code begin */
void
	SD_AXIv2_DummyGraphicsMacroModel::doneSendAW(void)
{
	if(AXIv2_master_TMaster->getSig(AW_VALID) && AXIv2_master_TMaster->getSig(AW_READY))
	{
		...
		case 64:
			core_64->m_transreceiver[0]->popAWTransaction();
			break;
		default:
			message(eslapi::CASI_MSG_WARNING,
				"SD_AXIv2_DummyGraphicsMacroModel::doneSendAW: p_Data_Bit_Width <%d> is not supported.",
				p_Data_Bit_Width);
			break;
		}
	}
...
void
	SD_AXIv2_DummyGraphicsMacroModel::doneSendW(void)
{
	if(AXIv2_master_TMaster->getSig(W_VALID) && AXIv2_master_TMaster->getSig(W_READY))
	{
		...
		switch(p_Data_Bit_Width){
		case 64:
			core_64->m_transreceiver[0]->popWTransaction();
			break;
		default:
			message(eslapi::CASI_MSG_WARNING,
				"SD_AXIv2_DummyGraphicsMacroModel::doneSendW: p_Data_Bit_Width <%d> is not supported.",
				p_Data_Bit_Width);
			break;
		}
	}
...
void
	SD_AXIv2_DummyGraphicsMacroModel::doneSendAR(void)
{
    ...
    std::cout << sc_time_stamp() << " doneSendAR" << std::endl;
		...
		switch(p_Data_Bit_Width){
		case 64:
			core_64->m_transreceiver[0]->popARTransaction();
			break;
		default:
			message(eslapi::CASI_MSG_WARNING,
				"SD_AXIv2_DummyGraphicsMacroModel::doneSendAR: p_Data_Bit_Width <%d> is not supported.",
				p_Data_Bit_Width);
			break;
		}
	}
...
void
	SD_AXIv2_DummyGraphicsMacroModel::getsendAW(void)
{
	switch(p_Data_Bit_Width){
	case 64:
		{
			axi_payload<DATA_WIDTH_64>* aw_transaction = NULL;
			aw_transaction = core_64->m_transreceiver[0]->frontAWTransaction();
			if(aw_transaction)
			{
	...
	default:
		message(eslapi::CASI_MSG_WARNING,
			"SD_AXIv2_DummyGraphicsMacroModel::getsendAW: p_Data_Bit_Width <%d> is not supported.",
			p_Data_Bit_Width);
		break;
	}
}
...
void
	SD_AXIv2_DummyGraphicsMacroModel::getsendW(void)
{
	...
	case 64:
		{
			...
			w_transaction = core_64->m_transreceiver[0]->frontWTransaction();
			if(w_transaction)
			{
				...
				if(tmp_wstrb & 0x0F)
				{
					AXIv2_master_TMaster->setWData((uint32_t)tmp_wdata, 0);
				}
				else
				{
					AXIv2_master_TMaster->setWData(0x00000000, 0);
				}

				if(tmp_wstrb & 0xF0)
				{
					tmp_wdata >>= 32;AXIv2_master_TMaster->setWData((uint32_t)tmp_wdata, 1);
				}
				else
				{
					AXIv2_master_TMaster->setWData(0x00000000, 1);
				}
	...
	default:
		message(eslapi::CASI_MSG_WARNING,
			"SD_AXIv2_DummyGraphicsMacroModel::getsendW: p_Data_Bit_Width <%d> is not supported.",
			p_Data_Bit_Width);
		break;
	}
...
void
	SD_AXIv2_DummyGraphicsMacroModel::getsendB(void)
{
	//uint8_t tmp_bid   = 0x0;
		...
		case 64:
			{
				...
				core_64->m_transreceiver[0]->pushBTransaction(b_transaction);
			}
			break;
		default:
			message(eslapi::CASI_MSG_WARNING,
				"SD_AXIv2_DummyGraphicsMacroModel::getsendB: p_Data_Bit_Width <%d> is not supported.",
				p_Data_Bit_Width);
			break;
		}
...
void
	SD_AXIv2_DummyGraphicsMacroModel::getsendAR(void)
{
	switch(p_Data_Bit_Width){
	case 64:
		{
			axi_payload<DATA_WIDTH_64>* ar_transaction = NULL;
			ar_transaction = core_64->m_transreceiver[0]->frontARTransaction();
			if(ar_transaction)
			{
#if 0
	...
	default:
		message(eslapi::CASI_MSG_WARNING,
			"SD_AXIv2_DummyGraphicsMacroModel::getsendAR: p_Data_Bit_Width <%d> is not supported.",
			p_Data_Bit_Width);
		break;
	}
}
...
void
	SD_AXIv2_DummyGraphicsMacroModel::getsendR(void)
{
	//uint8_t  tmp_rid      = 0x0;
		...
		case 64:
			{
				...
				tmp_rdata[0]  = AXIv2_master_TMaster->getRData(0);
				tmp_rdata[1]  = AXIv2_master_TMaster->getRData(1);
				...
				core_64->m_transreceiver[0]->pushRTransaction(r_transaction);
			}
			break;
		default:
			message(eslapi::CASI_MSG_WARNING,
				"SD_AXIv2_DummyGraphicsMacroModel::getsendR: p_Data_Bit_Width <%d> is not supported.",
				p_Data_Bit_Width);
			break;
		}
...
int
	SD_AXIv2_DummyGraphicsMacroModel::getDataWidth(void)
{
	return p_Data_Bit_Width;
}

void
	SD_AXIv2_DummyGraphicsMacroModel::controlInterrupt(void)
{
	...
	//std::cout << "[INFO]SD_AXIv2_DummyGraphicsMacroModel::controlInterrupt interrupt = " << (fj::uint32)interrupt_value 
	//          << " at " << sc_time_stamp() << std::endl;
}

void 
	SD_AXIv2_DummyGraphicsMacroModel::controlOutput(void)
{
...
void SD_AXIv2_DummyGraphicsMacroModel::controlRready(void)
{
	if(core_64->m_kernel->isValidateRreadyControl() == true)
	{
		AXIv2_master_TMaster->m_ar_done_flag = core_64->m_kernel->getRreadyValue();
	}
	else
	{
		if(m_r_count != 0)
...
void SD_AXIv2_DummyGraphicsMacroModel::init()
{
	...
	switch(p_Data_Bit_Width){
	case 64:
		core_64 = new DummyGraphicsMacroModel_wrapperbase<DATA_WIDTH_64>(p_model_id,
                                                                     p_number_of_thread,
    ...
    core_64->m_transreceiver[0]->m_clk( *(getMxSCClock()) );
		break;
	default:
		message(eslapi::CASI_MSG_WARNING,
...
void SD_AXIv2_DummyGraphicsMacroModel::reset(eslapi::CASIResetLevel level, const CASIFileMapIF *filelist)
{
...
void SD_AXIv2_DummyGraphicsMacroModel::terminate()
{
...
void SD_AXIv2_DummyGraphicsMacroModel::setParameter( const string &name, const string &value )
{
  ...
  else if (name == "model_id")
  {
	...
	{
		message( eslapi::CASI_MSG_WARNING, "SD_AXIv2_DummyGraphicsMacroModel::setParameter: Illegal value <%s> passed for parameter <%s>. Assignment ignored.", value.c_str(), name.c_str() );
	}
}

string SD_AXIv2_DummyGraphicsMacroModel::getProperty( eslapi::CASIPropertyType property )
{
	string description; 
	switch ( property ) 
	{    
	...
	case eslapi::CASI_PROP_DESCRIPTION:
		description = "SD_AXIv2_DummyGraphicsMacroModel";
...
string SD_AXIv2_DummyGraphicsMacroModel::getName(void)
{
	return "SD_AXIv2_DummyGraphicsMacroModel";
}

bool
	SD_AXIv2_DummyGraphicsMacroModel::saveData( CASIODataStream &data )
{
...
bool
	SD_AXIv2_DummyGraphicsMacroModel::restoreData( CASIIDataStream &data )
{
...
void
	SD_AXIv2_DummyGraphicsMacroModel::initSignalPort(CASISignalMasterIF* signalIf)
{
...
void
	SD_AXIv2_DummyGraphicsMacroModel::initAXIv2TransactionPort(CASITransactionMasterIF* transIf)
{
...
/************************
* SD_AXIv2_DummyGraphicsMacroModel Factory class
***********************/
class SD_AXIv2_DummyGraphicsMacroModelFactory : public CASIFactory
{
public:
	SD_AXIv2_DummyGraphicsMacroModelFactory() : CASIFactory ( "SD_AXIv2_DummyGraphicsMacroModel" ) {}
	CASIModuleIF *createInstance(CASIModuleIF *c, const string &id)
	{ 
		return new SD_AXIv2_DummyGraphicsMacroModel(c, id); 
	}
};
...
extern "C" void 
	CASIInit(void)
{
	new SD_AXIv2_DummyGraphicsMacroModelFactory();
}
