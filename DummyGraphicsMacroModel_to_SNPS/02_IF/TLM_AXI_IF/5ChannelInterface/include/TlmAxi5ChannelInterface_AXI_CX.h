...
class TlmAxi5ChannelInterface_AXI_CX: public sc_module,
                                      public BusTransactionInitiatorInterfacePrimitiveBase<DataType>
{
private:
  typedef boost::shared_ptr<axi_payload<DataType> > AxiPayloadPtr;

public:
  SC_HAS_PROCESS(TlmAxi5ChannelInterface_AXI_CX);
   ...
   **/
  TlmAxi5ChannelInterface_AXI_CX(const sc_module_name name,
...
private:
   ...
   * Bペイロードキューに格納する。
   * param[in] axi_payload Bペイロード
   */
  void receiveBRequest(axi_payload<DataType>* axi_payload_b);
   ...
   * Rペイロードキューに格納する。
   * param[in] axi_payload Rペイロード
   */
  void receiveRRequest(axi_payload<DataType>* axi_payload_r);
  
public:
  BusTransactionInitiatorInterfacePrimitive<DataType> m_primitive; /**< TLMソケット(TOP接続のため、public) */
...
private:
  ...
  AXIDataFormatControl m_format_controller; /**< AXIデータフォーマット変換クラス */
};
